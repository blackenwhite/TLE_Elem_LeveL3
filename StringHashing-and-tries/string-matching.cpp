...
void solve() {
    ...
    Hashing h1 = Hashing(n);
    Hashing h2 = Hashing(m);


    // for(int i=0; i < h1.powersOfP.size();i++){
    //     int ans = mult(h1.powersOfP[i],h1.inversePowersOfP[i]);
    //     cout<<ans<<"\n";
    // }
    int y = h2.getHash(0,m.size()-1);
    int ans = 0;
    for(int i=0;i<n.size()-m.size()+1;i++){
        int x = h1.getHash(i,i+m.size()-1);
        if(x==y) ans++;
    }
    cout<<ans<<"\n";
