...
void solve()
{
    ...
    int prod = 1;
    for(;left<distinct.size();left++) {
        while(right<distinct.size() && distinct[right]-distinct[left]<m){
            prod = mult(prod,mp[distinct[right]]);
            right++;
        }
        ...
        prod = mult(prod, inverse(mp[distinct[left]]));
    }
    cout<<ans<<"\n";
